...
def main(x, P_iv, Lqqiv, Lzziv, iriv, oriv, model, filename, path):
    ...
    def P_Lqiv(z, x, Lzz_iv, ir_iv, or_iv, Pa2Hg):
        ...
        if model == 'NH_2FF':
            ...
            I4_iv = Cqq_iv*(np.sin(x[5]))**2 + Czz_iv*(np.cos(x[5]))**2
            A = 0.5*(x[0]*Pa2Hg)
            C = 0.5*(x[2] + x[0] - 4.0*x[3])*Pa2Hg
            B = 0.5*(x[0] - x[3])*Pa2Hg
            W1 = 0.5*A*np.exp(x[1]*(I1_iv - 3.0))
            W4 = C*(I4_iv**-0.5)*((I4_iv**0.5) - 1.0)*np.exp(x[4]*((I4_iv**0.5) - 1.0)**2) + 2.0*B
            W5 = -B
            P_th = 2.0*(W1*(Cqq_iv - Crr_iv)
                        + (Cqq_iv*(np.sin(x[5]))**2)*(W4 + 2.0*W5*Cqq_iv))*np.log(or_iv/ir_iv)
            
        elif model == 'gSRM':
            # x[0]=u_T, x[1]=E_L, x[2]=u_L, x[3]=alpha
            I4_iv = Cqq_iv*(np.sin(x[3]))**2 + Czz_iv*(np.cos(x[3]))**2
            A = 0.5*(x[0]*Pa2Hg)
            C = 0.5*(x[1] + x[0] - 4.0*x[2])*Pa2Hg
            B = 0.5*(x[0] - x[2])*Pa2Hg
            W1 = A
            W4 = C*(I4_iv - 1.0) + 2.0*B
            W5 = -B
            P_th = 2.0*(W1*(Cqq_iv - Crr_iv)
                        + (Cqq_iv*(np.sin(x[3]))**2)*(W4 + 2.0*W5*Cqq_iv))*np.log(or_iv/ir_iv)
    ...
    plt.savefig("%s\%s\%s\%s" % (path, filename, model, 'Piv_Lqiv'), dpi=300, bbox_inches="tight")
    # =============================================================================
